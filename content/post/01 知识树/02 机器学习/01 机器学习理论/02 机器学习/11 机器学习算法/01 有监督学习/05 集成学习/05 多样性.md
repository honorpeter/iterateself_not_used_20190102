...
![mark](http://images.iterate.site/blog/image/180628/Eb0kFhjE5B.png?imageslim)

则集成的“分歧”是

![mark](http://images.iterate.site/blog/image/180628/l8CCC4a4GB.png?imageslim)

显然，这里的“分歧”项表征了个体学习器在样本 x 上的不一致性，即在 一定程度上反映了个体学习器的多样性。个体学习器 $h_i$ 和集成 H 的平方误差分别为

![mark](http://images.iterate.site/blog/image/180628/d8JlbkLfhi.png?imageslim)


令 $\overlien{E}(h|x)=\sum_{i=1}^{T}w_i\cdot E(h_i|x)$ 表示个体学习器误差的加权均值，有

![mark](http://images.iterate.site/blog/image/180628/mj671Fme53.png?imageslim)

式(8.31)对所有样本 x 均成立，令 $p(x)$ 表示样本的概率密度，则在全样本
上有

![mark](http://images.iterate.site/blog/image/180628/bgiE8aF85J.png?imageslim)

类似的，个体学习器 $h_i$ 在全样本上的泛化误差和分歧项分别为：

![mark](http://images.iterate.site/blog/image/180628/egbkc4A3L6.png?imageslim)

集成的泛化误差为

![mark](http://images.iterate.site/blog/image/180628/ak3BBALEa8.png?imageslim)


将式(8.33)~(8.35)代入式(8.32)，再令 $\overline{E}=\sum_{i=1}^{T}w_iE_i$ 表示个体学习器泛化误差的加权均值，$\overline{A}=\sum_{i=1}^{T}w_iA_i$ 表示个体学习器的加权分歧值，有

![mark](http://images.iterate.site/blog/image/180628/C0h9C0D0Gh.png?imageslim)
...
![mark](http://images.iterate.site/blog/image/180628/jhAGJELC9I.png?imageslim)


其中，a 表示 $h_i$ 与 $h_j$ 均预测为正类的样本数目；b、v、d 含义由此类推; a+b+c+d=m 。基于这个列联表，下面给出一些常见的多样性度量.

- 不合度量(disagreement measure)
![mark](http://images.iterate.site/blog/image/180628/kg3JBjH22e.png?imageslim)
$dis_{ij}$ 的值域为[0，l].值越大则多样性越大

- 相关系数(correlation coefficient)
![mark](http://images.iterate.site/blog/image/180628/h93e69K7g8.png?imageslim)
$\rho_{ij}$ 的值域为[-1，1].若 $h_i$ 与 $h_j$ 无关，则值为 0;若 $h_i$ 与 $h_j$ 正相关则值为正，否则为负.

- Q-统计量(Q-statistic)
![mark](http://images.iterate.site/blog/image/180628/C0g5baf83K.png?imageslim)
$Q_{ij}$ 与相关系数 $\rho_{ij}$  符号相同，且 $|Q_{ij}|\leq |\rho_{ij}|$ .

- $\kappa$ 统计量($\kappa$-statistic)
![mark](http://images.iterate.site/blog/image/180628/HgedF2bdfi.png?imageslim)
其中， $p_1$ 是两个分类器取得一致的概率; $p_2$ 是两个分类器偶然达成一致的概率，它们可由数据集 D 估算：
![mark](http://images.iterate.site/blog/image/180628/BFbh9he0K9.png?imageslim)
...
![mark](http://images.iterate.site/blog/image/180628/AcJ90lD57g.png?imageslim)
...
![mark](http://images.iterate.site/blog/image/180628/3aHcCkKK4m.png?imageslim)
